...
(function($, undefined) {
			...
			if (!this.$modal.length) {
				this.$modal = $(
						'<div class="modal fade" id="' + this.options.target.substr(1)
								+ '"><div class="modal-header"><a class="close" href="#" data-dismiss="modal">×</a><h3>&nbsp;</h3></div><div class="inner"/></div>').appendTo(this.options.appendTo)
						.hide();
			}
	...
	$.fn[pluginName].defaults = {
		...
		,
		content : '' // the static modal content (in case it's not loaded via
						// ajax)
		,
		appendTo : 'body' // where should the modal be appended to (default to
							...
							// needed in real life.
		,
		cache : false // should we cache the output of the ajax calls so that
						// next time they're shown from cache?
		,
		keyboard : false,
		nobackdrop : false
	};
